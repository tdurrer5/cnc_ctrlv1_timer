...
float constain(float, unsigned int, unsigned int); //tdu
...
int   coordinatedMove(const float& xEnd, const float& yEnd, const float& zEnd, float MMPerMin){
    ...
    float  zStartingLocation = sys.zPosition;
...
void  singleAxisMove(Axis* axis, const float& endPos, const float& MMPerMin){
    ...
    */
    ...
    long finalNumberOfSteps    = (long) abs(moveDist/stepSizeMM);      //number of steps taken in move
    finalNumberOfSteps = (long) abs(finalNumberOfSteps);
...
//    }
    ...
    if ((sign(theta) != sign(direction)) || ((abs(chordHeight) < .01) && (abs(theta) < 0.5)) || (radius > 25400)) {
      ...
      UARTprintf("Large-radius arc replaced by straight line to improve accuracy: %s", gcodeSubstitution);
